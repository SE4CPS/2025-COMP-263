...
(async () => {
  ...
  try {
    ...
    const docs = result.records.map(rec => {
      const farm = rec.get('farm'); const device = rec.get('device'); const reading = rec.get('reading');
      return {
        author: AUTHOR, studentId: STUDENTID, lab: 'lab3', course: 'DBMS2',
        sourceDB: 'Neo4j', ingestedAt: now,
        tags: ['agri','neo4j', `author:${AUTHOR}`, `sid:${STUDENTID}`],
        farm: { id: farm.id, name: farm.name, location: farm.location },
        device: { id: device.id, type: device.type, model: device.model },
        reading: { rid: reading.rid, metric: reading.metric, value: reading.value, unit: reading.unit, ts: reading.ts ? reading.ts.toString() : null }
      };
    });
    ...
    await session.close(); await neoDriver.close(); await mclient.close();
  }
})();