...
const mergedDocs = READINGS
  .map(r => {
    // lookup sensor by sensor_id
    const s = sensorById[String(r.sensor_id)];
    if (!s) return null; // skip readings without matching sensor
...
const enrichedDocs = mergedDocs.map(doc => {
  // checksum over stable subset of fields
  ...
  const metadata = {
    ...
    record_source: 'lab5_starter.js',
    transform_status: 'merged_and_enriched'
  };
...
const shards = enrichedDocs.reduce((acc, d) => {
  const farmId = d.farmId || 'UNKNOWN_FARM';
