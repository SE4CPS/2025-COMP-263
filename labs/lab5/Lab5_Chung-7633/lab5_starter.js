...
// HINT: use READINGS.map(...).filter(Boolean)
const mergedDocs = READINGS
  .map(r => {
    ...
    // TODO: lookup sensor `s` by r.sensor_id
    // const s = ...
    // if (!s) return null;

    // TODO: build the EXACT Q1 schema (`coreDoc`)
    // const coreDoc = { _id: randomUUID(), farmId: ..., deviceId: ..., crop: ..., gps: {lat:..., lon:...}, ts_utc: r.ts_utc, soil_moisture: r.soil_moisture, temp_c: r.temp_c, battery_v: r.battery_v };
...
const enrichedDocs = mergedDocs.map(doc => {
  ...
  const metadata = {
    uuid: doc._id,
    checksum_md5,
    author: "Cheng Han Chung(HANK)",
    sync_time_utc: new Date().toISOString(),
    source_db: "sql_cluster_A",
    ...
    quality_flags: {
      battery_ok: doc.battery_v > 3.5,
      value_valid: !isNaN(doc.soil_moisture)
    }
  };

  return { ...doc, metadata };
  // TODO: compute checksum_md5
  ...
  // TODO: construct metadata { uuid, author, sync_time_utc, ... }
  // const metadata = { ... };

  // return { ...doc, metadata };
  //return doc; // <-- replace with enriched
});
...
const shards = enrichedDocs.reduce((acc, d) => {
  ...
  // TODO: derive shardName using farmId + toDay(ts_utc)
  // const shardName = ...
  // acc[shardName] = acc[shardName] ? acc[shardName].concat([d]) : [d];
  return acc;
}, {});
