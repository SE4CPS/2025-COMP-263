 ...
 * FINAL SOLUTION — Q1: merge, Q2: metadata, Q3: sharding
 *
 * Files expected:
 *  - sql_sensors.json
...
// ---------- (Q1): Join & merge with EXACT schema -----------------
/**
...
// ---------- (Q2): Enrich with ≥10 metadata fields ----------------
/**
...
const enrichedDocs = mergedDocs.map(doc => {
  // 1. Create stable payload for checksum
  ...
  // 3. Build metadata object with ≥10 fields
  const metadata = {
    uuid: doc._id,
    checksum_md5,
    author: 'lab5-student',
    sync_time_utc: new Date().toISOString(),
    source_db: 'sql_farm_telemetry',
    source_tables: ['sensors', 'readings'],
    ingest_batch_id: 'lab5-batch-001',
    ...
    record_source: 'sql_to_nosql_migration',
...
// ---------- (Q3): Shard with reduce (partition by farmId+day) ----
/**
 * Partition into shards named: readings_<farmId>_<YYYY-MM-DD>
 * Shard key logic:
 *   shardKey = farmId + "_" + dayBucket
 * where dayBucket is YYYY-MM-DD from ts_utc.
 *
 * This groups data by farm (tenant) and day, which:
 *  - keeps each shard a reasonable size,
 *  - matches common queries like "all readings for FARM-001 on 2025-11-01",
 *  - gives good locality for time-bounded analytics per farm.
 */

const toDay = (iso) => (typeof iso === 'string' ? iso.slice(0, 10) : 'UNKNOWN');

const shards = enrichedDocs.reduce((acc, d) => {
  const day = toDay(d.ts_utc); // e.g. "2025-11-01"
