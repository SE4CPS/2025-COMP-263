...
const enrichedDocs = mergedDocs.map(doc => {
  const payload = JSON.stringify(pick(doc, [
    "farmId", "deviceId", "ts_utc", "soil_moisture", "temp_c", "battery_v"
  ]));
  ...
  const metadata = {
    ...
    source_db: "sql_farm_v1",
    source_tables: ["sql_sensors", "sql_readings"],
    ingest_batch_id: "batch-" + new Date().toISOString().slice(0,10),
    lineage: `${doc.deviceId}/${doc.__raw?.reading_id ?? "unknown"}`,
    units: {
      soil_moisture: "%",
      temp_c: "C",
      battery_v: "V"
...
const shards = enrichedDocs.reduce((acc, d) => {
  const dateStr = toDay(d.ts_utc);
  const farmId = d.farmId;
  const shardName = `readings_${farmId}_${dateStr}`;

  if (!acc[shardName]) {
    acc[shardName] = [];
  }

  acc[shardName].push(d);
  return acc;
}, {});
