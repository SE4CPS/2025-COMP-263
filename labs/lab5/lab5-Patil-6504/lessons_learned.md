...
A good shard key (like farmId + date) must distribute evenly, preventing hot shards by spreading writes across time and location dimensions. Allows adding new shards as data grows without rebalancing existing data. Queries filtering by farm and time can target specific shards efficiently
