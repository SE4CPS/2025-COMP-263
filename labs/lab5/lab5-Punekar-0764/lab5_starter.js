...
const mergedDocs = READINGS
  .map(r => {
    // join reading with its sensor row
    const s = sensorById[String(r.sensor_id)];
    if (!s) return null; // skip orphan readings

    // EXACT schema required by the question
    const coreDoc = {
      _id: randomUUID(),
      farmId: s.farm_id,
...
const enrichedDocs = mergedDocs.map(doc => {
  // checksum calculated on stable fields
  ...
  const metadata = {
    uuid: doc._id,
    checksum_md5,
    author: "Omkar",                 // or your name
    sync_time_utc: new Date().toISOString(),
    source_db: "sql_simulation_db",
    source_tables: ["sql_sensors", "sql_readings_100rows"],
    ingest_batch_id: "batch_2025_11_lab5",
    ...
    quality_flags: {
      low_battery: doc.battery_v < 3.6,
      abnormal_temp: doc.temp_c < -10 || doc.temp_c > 60
    },
    record_source: "Lab5_SQL_to_NoSQL_pipeline",
...
const shards = enrichedDocs.reduce((acc, d) => {
  const day = toDay(d.ts_utc);                 // "2025-11-01"
  const shardName = `readings_${d.farmId}_${day}`;

  acc[shardName] = acc[shardName]
    ? acc[shardName].concat([d])               // append to existing shard
    : [d];                                     // create new shard array
