...
const enrichedDocs = mergedDocs.map(doc => {
  ...
  const metadata = {
    ...
    quality_flags: {
      ...
      missing_temp: Number.isNaN(doc.temp_c)
    }
  };
...
const shards = enrichedDocs.reduce((acc, d) => {
  ...
  const prev = acc[shardName] || [];
  acc[shardName] = prev.concat(d);
  return acc;
}, {});
