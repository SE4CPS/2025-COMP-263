...
const enrichedDocs = mergedDocs.map(doc => {
  ...
  const metadata = {
    ...
    source_db: "agri_sql_db",
    source_tables: ["sensors", "readings"],
    ingest_batch_id: `BATCH-${doc.farmId}-${doc.ts_utc.slice(0,10)}`,
    ...
    quality_flags: {
      valid: doc.soil_moisture !== null &&
             doc.temp_c !== null &&
             doc.battery_v !== null,
      missing_fields: []
    },
    record_source: "lab5_functional_migration",
...
const shards = enrichedDocs.reduce((acc, d) => {
  const day = toDay(d.ts_utc); // YYYY-MM-DD
